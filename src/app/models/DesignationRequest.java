...
public abstract class DesignationRequest implements HttpRequestListener {
	...
	public void getDesignations(){
		dispatcher = new HttpRequestDispatcher(AirCrew.designation, method, requestListener, "");
		dispatcher.start();
	}
	...
	public void httpsuccess(byte[] array, String str) {
		...
		try{
			...
			if(json.has("Designation")) {
				...
				JSONArray response = json.getJSONArray("Designation");
				Designation[] designations = new Designation[response.length()];
				for(int i=0; i<response.length(); i++){
					JSONObject designation = (JSONObject)response.get(i);
					String id = designation.getString("id");
					String name = designation.getString("name");
					designations[i] = new Designation(id, name);					
				}
				afterSuccess(designations);
	...
	public abstract void afterSuccess(Designation[] designations);
}
